...
function handleNextButtonClick() {
  ...
  for (var i = 0; i < selectedHabits.length; i++) {
    ...
    break; // This might not be necessary based on your use case
  }
...
// on page refresh get habits from localstorage
...
function createHabitCard(
  ...
  // Add an id to the habitCard element
  habitCard.setAttribute("id", "habbitCard");
  ...
  function setActiveState(active) {
    const activeHabits = JSON.parse(localStorage.getItem("activeHabits")) || {};
    activeHabits[habitId] = active;
  ...
  function handleClick() {
    ...
    setActiveState(habitCard.classList.contains("active"));
  }
  ...
  deleteButton.addEventListener("click", function () {
    ...
    var habitIds = habitCard.getAttribute("data-habit-id");
    ...
    console.log("ids", habitIds);
    console.log(localStorage.getItem("habits"));
    // Find the index of the habit to delete
    const habitIndexToDelete = storedHabits.findIndex(
      (habit) => habit.habitId === habitId // Replace habitText with the actual text of the habit to delete
    );

    if (habitIndexToDelete !== -1) {
      // If the habit is found in the array, remove it
      storedHabits.splice(habitIndexToDelete, 1);

      // Update the habit count by subtracting 1
      habitCount -= 1;
      document.getElementById("added-habits").innerText = habitCount;
      console.log("index is ", habitIndexToDelete);
      // Save the updated array back to local storage
      localStorage.setItem("habits", JSON.stringify(storedHabits));
    }
  });

  // Add the initial "active" state based on localStorage when the card is created
  const activeHabits = JSON.parse(localStorage.getItem("activeHabits")) || {};
  if (activeHabits[habitId]) {
    habitCard.classList.add("active");
  }

  // Append the habit icon, text, input value, and delete button to the habit card

  // Add the click event listener
  habitCard.addEventListener("click", handleClick);
  ...
  // Append the habit card to the habit list container
  habitCard.setAttribute("data-habit-id", habitId);
  console.log("id", habitId);
  habitListContainer.appendChild(habitCard);

  // Scroll to the bottom of the habit list container
  habitListContainer.scrollTop = habitListContainer.scrollHeight;
...
var heading1 = navbar.querySelector("h1");
...
function enableLightMode() {
  ...
  heading1.style.color = "#395B64";

  // Save the mode preference to localStorage
  localStorage.setItem("mode", "light");
}

// Function to check the user's mode preference from localStorage and apply it
...
// Check and apply the user's mode preference on page load
checkModePreference();
...
// Function to handle the click on habit cards
function handleHabitCardClick(event) {
  const habitCard = event.currentTarget;
  habitCard.classList.toggle("active"); // Toggle the "active" class on the clicked habit card

  // Get all habit cards and find the index of the clicked habit card
  const allHabitCards = document.querySelectorAll(".habbit-card");
  const index = Array.from(allHabitCards).indexOf(habitCard);

  // Store the active state in local storage with a unique key, e.g., "habit_active_state_1689779006033"
  localStorage.setItem(
    "habit_active_state_" + habitCard.dataset.habitId,
    habitCard.classList.contains("active")
  );
}

// Function to apply the active state on page load
function applyActiveStateOnLoad() {
  const habitCards = document.querySelectorAll(".habbit-card");
  habitCards.forEach((card) => {
    // Retrieve the previous active state from local storage and apply it on page load
    const isActive =
      localStorage.getItem("habit_active_state_" + card.dataset.habitId) ===
      "true";
    if (isActive) {
...
// Add click event listener to all habit cards
const habitCards = document.querySelectorAll(".habbit-card");
habitCards.forEach((card) => {
  card.addEventListener("click", handleHabitCardClick);
});

// Apply the active state on page load
applyActiveStateOnLoad();